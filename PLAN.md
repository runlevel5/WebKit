...
### Phase 3 status — 239/243 baseline crashes fixed (98%), 4 remain (2026-07-21)
...
Prime suspect for the tail-call crashes: `llint_op_tail_call` frame shuffling / `prepareForTailCall` on PPC (the tail-call path reuses the caller frame and must handle LR + the ELFv2 header differently than a normal call). Then re-run the full 500-file triage (remove the now-invalid `--maxDFGNodeCount=0` from `/tmp/triage.sh`) and run the formal Phase 3 gate.
