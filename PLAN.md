...
>
> **REMAINING — DFG-only call-linking bug (deterministic now, 10/10).** With baseline fixed, the DFG config (`--useDFGJIT=1 --thresholdForOptimizeAfterWarmUp=10`) still crashes deterministically once `f` tiers up to DFG (f3–f5 stay baseline → pass; f6+ → DFG → crash; DFG regular calls like `g→h` also crash). Signature: `pc=0` (null call target). The DFG's slow-path `callOperation` slot is **malformed** — the target is materialized (`lis/ori/sldi/oris/ori` = the `operationLinkDirectCall` address, e.g. into r0) but there is **NO `mtctr`/`bctrl`** before the post-call TOC restore. So the slot was linked as a **5-insn pointer (li64) not an 8-insn call slot (`applyCallSlot`, which puts `mtctr` at word[5], `bctrl` at word[7])** → execution loads the address and falls through → `pc=0`. Likely exposed (not caused) by the `call()`→Linkable flag change routing this DFG operation-call's linking down a path that writes only the address. NEXT: find where the DFG links its `callOperation` slots (JITCompiler/`m_calls` link step) and why a `Linkable` call there gets `linkPointer`/`writeLi64` treatment instead of `applyCallSlot`; compare to how BASELINE `callOperation`s link (those work with the same Linkable flag). This is the last thing between here and a working DFG for calls.
