...
> **Phase 4 grind progress (2026-07-29):** Fixes landed with JIT actually on — `212e0f2` branchConvertDoubleToInt32 (DFG rounding); `6af2020` **prepareForTailCall** PPC branch (CallFrameShuffler, `m_newFrameOffset=-2` + `restoreReturnAddressBeforeReturn` mtlr) → **fixed-arg tail calls + tail recursion work** (`sum(100,0)`=5050, more/fewer/no-arg tail calls all correct); `2e8e519` **branchSub64** (was UNIMPLEMENTED: non-overflow via sub64+branchTest64, Overflow via `(a^b)&(a^result)<0`) + **prepareForTailCallSlow** PPC branch (CCallHelpers, mirrors ARM64). **STILL OPEN (WIP/next):** (a) **varargs tail calls** (`g.apply(null,arguments)` in strict mode → op_tail_call_varargs) get past the UNREACHABLE + branchSub64 but SIGSEGV in the frame shuffle downstream — the `prepareForTailCallSlow` PPC frame math/copy loop needs validation (no regression: crashed before too). (b-RESOLVED, 2026-07-29) **The baseline named-property crash was TWO separate PPC64 codegen bugs, both now fixed and committed:**
  ...
  **STILL FAILING (narrower, shared root cause — NEXT):** `array.length` (a CustomValue getter) and 3-way+ polymorphic get_by_id. Both crash with an identical signature: sp climbs to the stack ceiling (~0x800000000000) over ~hundreds of calls (n≤100 ok, n≥500 crash), then a later handler faults doing `ld r31,0(r1)`/`ld r0,8(r1)` from the corrupt sp (r31/r0 load filesystem-path strings from bad memory). It is an **sp LEAK per fast-path call**, not an immediate imbalance. Every individual handler disassembled so far (GetById Custom handler, the ff-inline slow-path stub at ~0x…cf60) is sp-BALANCED with the fix (`push16+sub96(+sub96 makeSpace) … +96(+96)+pop16 = 0`), so the leaking path is not yet isolated — candidates: the array-length-specific access-case codegen, the polymorphic-list handler chain (`emitDataICJumpNextHandler` farJump), or an IC repatch/transition path that runs a restore without a matching prepare. Disassemble the actual fast handler installed for array.length after IC setup (break once mapped and print r1 across calls to catch the drift), or grep for an `emitDataICRestoreAfterCall` reachable without `emitDataICPrepareForCall`. [Original investigation trail — superseded but kept for method:]
