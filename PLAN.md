...
**Triage method that works** (used for the mulp bug): gdb the SIGSEGV → note the faulting instruction and that it's in LLInt glue vs JIT code → confirm JIT-on-specific (compare `--useJIT=0`) → open the source-annotated generated `LLIntAssembly.h` (grep the function label, the `// LowLevelInterpreter*.asm:NNNN` comments map each instruction to source) → spot the wrong register/opcode → fix the ppc64le.rb lowering or the MacroAssembler method. **Next session: grind the baseline stress crashes**, then run the formal Phase 3 gate (full stress at baseline via a proper runner invocation forcing `--useDFGJIT=0`). The crashes triage into (at least) two distinct classes:
- **Arithmetic/opcode miscompiles** — like the mulp bug; fix in ppc64le.rb / MacroAssembler.
- **Stack-walk / JIT-frame-metadata** — triaged `add-overflow-check-with-three-same-registers.js` (repro with a 5-line harness defining `noInline`/`testLoopCount`): the abort is a `RELEASE_ASSERT` in `WTFCrashWithInfoImpl` ← `StackVisitor::StackVisitor` ← `CallFrame::callerSourceOrigin` ← `constructFunction` (the test's `Function(...)` ctor). The stack visitor walks up from a baseline-JIT frame (frame #8 is in the JIT region) and asserts on a frame it can't parse — i.e. the baseline frame doesn't present the layout the unwinder expects (codeBlock slot, return-PC classification, or callerFrame link). This is a frame-contract bug, NOT arithmetic. Likely shared with DFG OSR later, so worth fixing properly. Note: the harness's minimal `noInline` stub means the tested function may actually get inlined-ish behavior differences — re-triage under the real runner to be sure, but the StackVisitor assertion is a genuine baseline-frame signal.
