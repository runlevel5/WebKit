...
**NEXT BUGS (not fixed):** (1) **DFG direct-call linking** — recursion (`fib`) crashes: `operationLinkDirectCall` hits the `JSObject::realm()` RELEASE_ASSERT (JSObject.h:575, "realmless structure") on a corrupt callee — the SAME signature as the Phase-3 exception-unwind ELFv2 bug, so suspect another tiny-frame C-call linkage-area collision in a DFG call thunk (or a `DirectCallLinkInfo::emitDirectFastPath` / `storePtrWithPatch` frame-setup error; check the callee slot the way the Phase-3 GUPROBE did). (2) **reentrant natives** (`sort` with a comparator) crash — related call-path issue. (3) `moveWithPatch(TrustedImm32)`/`branchPtrWithPatch` still stubs — implement when hit (need `repatchInt32`/`readInt32` for the 2-insn lis+ori form). Then run the Phase-4 gate (stress at Baseline+DFG, `--useFTLJIT=0`).
