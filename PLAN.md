...
### Phase 3 status — 242/243 baseline crashes fixed (99.6%), 1 remains (2026-07-21)

**UPDATE: the tag-swap fix (below) cleared 3 more, including both `array-flatmap.js` and `array-iterators-next-with-call.js`. Only 1 of the original 243 baseline-only crashers remains: `allocation-sinking-puthint-control-flow.js`** — a `WTFCrashWithInfo` abort in a DFG-allocation-sinking test (uses `noInline`; likely exercises the compiled-but-not-brought-up DFG even at `--useDFGJIT=0`, or a distinct bytecode assertion — get the exact assert with an assertion build and check whether it reproduces with DFG genuinely disabled). This is unrelated to the tail-call/tag clusters. Next: characterize it, then re-run the full 500-file triage and the formal Phase 3 gate.
