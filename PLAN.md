...
**Grind progress (2026-07-16)**: JSC::initialize fully passes with the JIT live — all LLInt glue thunks emit and run, including the ELFv2 C-call round-trip (r2 saved at sp+24 inside the 96-byte maxFrameExtentForSlowPathCall reservation; was wrongly 0). **testmasm: 49 tests passing** (`JSC_useJIT=1`, stubs self-identify via PPC64_UNIMPLEMENTED). Landed: call family (compile-time PtrTag discrimination), full FP surface (classic PPC float model, CR-logic single-branch DoubleConditions via cror/crandc), conditional moves (aliasing-safe bc-skip), 64-bit shifts, signed loads, pairs, memory-form arith. testmasm caught two real bugs: getEffectiveAddress base-aliasing and resolveAddress(BaseIndex) addis-overflow on offsets ≥ 0x7FFF8000. **Next: the Probe API trampoline** (ctiMasmProbeTrampoline in MacroAssemblerPPC64.cpp — full CPU-state save/call/restore honoring the pc/cr/xer/lr/ctr ProbeContext), then the remaining testmasm families, then jsc baseline.
