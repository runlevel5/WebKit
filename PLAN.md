...
(1) **DFG: two direct calls to the same callee corrupt the callee frame — PRECISELY LOCALIZED (2026-07-25), not yet fixed.** Repro: `function f(n){return n<2?n:f(n-1)+f(n-1)} noInline(f); for(i<50)f(3)` crashes; `f(n-1)+7` (ONE direct call) WORKS; linear/mutual recursion WORK. The crash surfaces as the `JSObject::realm()` RELEASE_ASSERT (JSObject.h:575) inside `operationLinkDirectCall`'s epilogue (`OPERATION_RETURN`/tracer reading the caller frame's callee) — a MISLEADING symbol; the real fault is that the **child f frame's callee slot holds `0x2000` (a call-site-index-like value) instead of the callee JSFunction**. Proven with a `getenv("PPC_DIRECTCALL_PROBE")` C++ probe in `operationLinkDirectCall` (DFGOperations.cpp): the corrupt callee is present AT the operation's ENTRY (so it's DFG CODEGEN corruption, NOT the C-call/ELFv2, NOT `linkDirectCall`/`repatchCall` — those complete fine). The callee is stored by `store64(calleeGPR, calleeFrameSlot(CallFrameSlot::callee))` (DFGSpeculativeJIT64.cpp:933, common path) → `[sp+8]` → child's `[cfr+24]`; so **`calleeGPR` holds `0x2000` (the call-site index) for the SECOND of two direct calls** instead of the frozen callee constant. VERIFIED CORRECT (do NOT re-investigate): TagOffset=4/PayloadOffset=0 (LE), `emitStoreCallSiteIndex`→`tagFor(argCount)`=`[cfr+36]`, `calleeFrameSlot`→`[sp+8]`, frame layout, the GP register callee-save flags (r3–r10 volatile, r14–r31 non-volatile — PPC64Registers.h), patchable moves + near-call patching (applyCallSlot/relinkCall). **NEXT: disassemble f's DFG code (gdb native PPC disasm on the live process; get the entry from `codePtr` via the probe) and find where the 2nd call's `calleeGPR` gets `0x2000` — prime suspect is a register-liveness/silent-spill gap where the callee constant isn't re-materialized after the first call clobbers its register, or the call-site-index materialization (dataTemp r11) leaking into calleeGPR. Watchpoints fight stack-reuse (the callee-slot stack address is also used by C++ HashTable::rehash) — constrain to JIT-range pc, or watch the register in the DFG code directly.**

(2) **reentrant natives** (`sort` with a comparator) crash — related call-path issue, check after (1). (3) `moveWithPatch(TrustedImm32)`/`branchPtrWithPatch` still stubs — implement when hit (need `repatchInt32`/`readInt32` for the 2-insn lis+ori form). Then run the Phase-4 gate (stress at Baseline+DFG, `--useFTLJIT=0`).
